...
<title>Update Account — FinTech CMS</title>
<style>
:root{
...
input:focus ~ .input-emoji, select:focus ~ .input-emoji{
...
<p class="lead">Provide Account ID and update balance or status. 💼</p>
...
<form id="updateAccountForm" novalidate>
  ...
  <div class="input-group">
    ...
    <div class="input-wrapper">
      <input id="balance" name="balance" type="number" step="0.01" min="0" placeholder="e.g. 1200.50">
      <span class="input-emoji">💵</span>
    </div>
    <div class="input-hint">
      <span class="input-hint-emoji">💡</span>
      <span>Leave empty to keep current balance</span>
    </div>
  </div>
  ...
  <div class="input-group">
    ...
    <div class="input-hint">
      <span class="input-hint-emoji">💡</span>
      <span>Leave empty to keep current status</span>
    </div>
  </div>
...
(function(){
  ...
  [accountIdInput, balanceInput].forEach(input => {
  ...
  form.addEventListener('submit', function(e){
    ...
    formData.append('accountId', accountIdInput.value);
    if(balanceInput.value.trim() !== '') formData.append('balance', balanceInput.value);
    if(statusSelect.value !== '') formData.append('status', statusSelect.value);

    const originalText = btnText.innerHTML;
    btnText.innerHTML = 'Updating<span class="loading"></span>';

    fetch('../Imp/Account/updateAccountImp.php', {
      method: 'POST',
    ...
    .then(data => {
      ...
      if(data.success){
        createConfetti();
        const statusEmoji = data.account.status === 'Active' ? '✅' : '⏸️';
        ...
        <div class="account-card">
          ...
          <ul>
            <li>
              ...
              <strong>ID:</strong> ${data.account.id}
            </li>
            <li>
              <span class="item-emoji">${statusEmoji}</span>
              <strong>Status:</strong> ${data.account.status}
            </li>
            <li>
              <span class="item-emoji">💰</span>
              <strong>Balance:</strong> $${parseFloat(data.account.balance).toFixed(2)}
            </li>
          </ul>
        </div>`;
      } else {
  ...
  resetBtn.addEventListener('click', function(){
    form.reset();
    resetMessages();
    [accountIdInput, balanceInput, statusSelect].forEach(input => {
